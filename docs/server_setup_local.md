...
For other remote server setups, see the [sshfs server setup tutorial](server_setup_sshfs.md). You can also use Dropbox instead of sshfs.
...
2.  To run the trainer. This will first create the sync directory.
    ...
    Note: if you are installing the RootPainter trainer (server) from scratch on windows 11 I suggest [these linked instructions](windows_11_trainer_install.md).
    ...
    For an example of how to use RootPainter to train a model, see the [lung tutorial](cxr_lung_tutorial.md). It is now recommended to follow the [Colab tutorial](https://colab.research.google.com/drive/104narYAvTBt-X4QEDrBSOZm_DRaAKHtA?usp=sharing) instructions but using your local setup instead of the colab server, as these are easier to follow than the lung tutorial.
