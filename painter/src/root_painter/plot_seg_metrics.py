...
class Thread(QtCore.QThread):
    ...
    def run(self):
        ...
        for i, fname in enumerate(self.fnames):
            ...
            # recompute metrics, just in case the segmentation now exists.
            if metrics is None:
                metrics = compute_seg_metrics(
                    self.seg_dir, self.annot_dir, fname, self.model_dir, annot_events
                )
...
class QtGraphMetricsPlot(QtWidgets.QMainWindow):
    ...
    def __init__(self, fnames, metrics_list, rolling_n, selected_fname=None):
        ...
        self.control_bar_layout.setContentsMargins(
            10, 0, 0, 10
        ...
        self.combo_container_layout.setContentsMargins(
            0, 0, 0, 0
        )  # left, top, right, bottom
    ...
    def add_selected_point_label(self):
        ...
        selected_point_widget_layout.setContentsMargins(
            0, 0, 0, 0
        )  # left, top, right, bottom
        self.navigate_btn.hide()
        selected_point_widget.setContentsMargins(
            0, 0, 10, 0
        )  # left, top, right, bottom
    ...
    def add_first_corrective_control(self):
        ...
        self.first_cor_spin = pg.SpinBox(
            value=default_first_corrective_idx() + 1,
            bounds=[1, 1000],
            int=True,
            minStep=1,
            step=1,
    ...
    def set_highlight_point(self, highlight_point_fname):
        ...
        if self.highlight_point_fname in self.fnames:
            ...
            self.selected_point_label.setText(f"Y: {round(y, 4)}")
    ...
    def render_highlight_point(self):
        if self.show_selected:
            if self.highlight_point_fname in self.fnames:
                ...
                if hasattr(self, "corrected_dice") and not math.isnan(
                    self.corrected_dice[idx]
                ):
                    x = [self.x_vals[idx]]
                    y = [self.metrics_list[idx][self.selected_metric]]
                    if self.highlight_point is not None:
                        self.highlight_point.setData(
                            x, y
                        )  # update position of existing point clearing causes trouble.
                    else:
                        self.highlight_point = self.graph_plot.plot(
                            x,
                            y,
                            symbol="o",
    ...
    def render_data(self):
        ...
        if not [c for c in self.corrected_dice if not math.isnan(c)]:
            ...
            message = (
                "Segmentation metrics are not yet available because correctively"
            ...
            if self.x_axis_display_name == "Image":
                x = list(
                    range(1, len(self.corrected_dice) + 1)
                )  # start from 1 because first image is 1/N
            ...
            scatter_points = [
                {"pos": [x, y], "data": f} for (x, y, f) in zip(x, y, self.fnames)
            ]
            self.graph_plot.clear()
            ...
            self.graph_plot.plot(
                ...
                symbol=None,
                name=f"Average (n={self.rolling_n})",
            )
...
def demo_plot_test():
    ...
    corrected_dice += [0.7]
    metrics_list = [
        {"f1": a, "accuracy": (1 - a) / 2, "annot_fg": 100, "annot_bg": 100}
        for a in corrected_dice
    ]
    rolling_n = 3
