...
class CreationProgressWidget(BaseProgressWidget):
    ...
    def run(self, all_image_paths, output_dir, resize_percent):
        ...
        self.creation_thread = CreationThread(
            all_image_paths, output_dir, resize_percent
        )
    ...
    def run(self):
        ...
        for fpath in self.all_image_paths:
            ...
            output_path = os.path.join(
                self.output_dir, os.path.splitext(fname)[0] + ".jpg"
            )
...
class ResizeWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        edit_resize_percent_label.setText(
            "Resize percentage (target width and height):"
        )
        ...
        info_label.setText(
            "Source directory, output directory and resize percentage must"
    ...
    def validate(self):
        ...
        if not self.resize_percent_edit_widget.value():
            self.info_label.setText("Resize percent must be specified to resize images")
            self.create_btn.setEnabled(False)
            return
