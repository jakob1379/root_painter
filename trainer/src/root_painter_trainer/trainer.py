...
from root_painter_trainer.metrics import (
    get_metrics,
    get_metrics_str,
    get_metric_csv_row,
)
...
class Trainer:
    def __init__(
        ...
        assert patch_size in valid_sizes, (
            f"Specified patch size of {patch_size}"
        ...
        if sync_dir:
            ...
            self.settings_path = os.path.join(Path.home(), "root_painter_settings.json")
        ...
        # These can be triggered by data sent from client
    ...
    def main_loop(self):
        print("Started main loop. Checking for instructions in", self.instruction_dir)
        while True:
            try:
                ...
                self.log(
                    f"Exception check_for_instructions instruction,{e},{traceback.format_exc()}"
                )
            if self.training:
    ...
    def fix_config_paths(self, old_config):
        ...
        for k, v in old_config.items():
            if k == "file_names" or k == "format":
                ...
                for e in v:
                    new_val = e.replace("\\", "/")
                    new_val = os.path.join(self.sync_dir, os.path.normpath(new_val))
                    new_list.append(new_val)
                new_config[k] = new_list
            elif isinstance(v, str):
                v = v.replace("\\", "/")
                new_config[k] = os.path.join(self.sync_dir, os.path.normpath(v))
            else:
                new_config[k] = v
        return new_config
    ...
    def start_training(self, config):
        if not self.training:
            ...
            self.train_set = TrainDataset(
                self.train_config["train_annot_dir"],
            ...
            self.optimizer = torch.optim.SGD(
                self.model.parameters(), lr=0.01, momentum=0.99, nesterov=True
            )
            self.model.train()
            self.training = True
    ...
    def train_one_epoch(self):
        ...
        defined_total = 0
        loss_sum = 0
        for step, (photo_tiles, foreground_tiles, defined_tiles) in enumerate(
            train_loader
        ):
            self.check_for_instructions()
    ...
    def log_metrics(self, name, metrics):
        ...
        if not os.path.isfile(fpath):
            ...
            print(
                "date_time,true_positives,false_positives,true_negatives,"
    ...
    def validation(self):
        ...
        Also stop training if the current model hasn't
        beat the previous model for {max_epochs}
        """
        model_dir = self.train_config["model_dir"]
        # TODO consider implementing checkpointer class to maintain
        # this state.
        get_val_metrics = partial(
            model_utils.get_val_metrics,
            val_annot_dir=self.train_config["val_annot_dir"],
        ...
        was_saved = save_if_better(
            model_dir, self.model, prev_path, cur_metrics["f1"], prev_metrics["f1"]
        )
        if was_saved:
            self.epochs_without_progress = 0
        ...
        message = (
            f"Training {self.epochs_without_progress}"
        ...
        if self.epochs_without_progress >= self.max_epochs_without_progress:
            message = (
                f"Training finished as {self.epochs_without_progress}"
                " epochs without progress"
            )
            print(message)
            self.log(message)
    ...
    def write_train_metrics(self, metrics):
        metric_str = get_metrics_str(
            metrics, to_use=["f1_score", "recall", "precision", "duration"]
        )
    ...
    def segment(self, segment_config):
        ...
        start = time.time()
        for fname in fnames:
            self.segment_file(in_dir, seg_dir, fname, model_paths, format_str)
    ...
    def segment_file(self, in_dir, seg_dir, fname, model_paths, format_str):
        ...
        try:
            if not self.training:
                ...
                if self.msg_dir:
                    ...
                    # then the current instruction might not be associated with
                    # any particular project, so do not send a message.
                    pass

        except Exception as e:
            stack = traceback.format_exc()
            print("exception writing message", e, stack)
        ...
        if not os.path.isfile(fpath):
            ...
            try:
                ...
                return
            seg_start = time.time()
            seg_out = ensemble_segment(
                model_paths, photo, self.bs, self.in_w, self.out_w
            )
