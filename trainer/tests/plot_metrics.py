...
def plot_metrics(
    ...
    def plot_file_set(
        train_files, val_files, train_color, val_color, linestyle, label_prefix
    ):
        ...
        for train_file, val_file in zip(train_files, val_files):
            ...
            # Read data
            train_x_values, train_y_values = read_csv(
                train_file, x_axis, start_time_str, metric
            )
            val_x_values, val_y_values = read_csv(
                val_file, x_axis, start_time_str, metric
            )
            ...
            plt.plot(
                ...
                alpha=0.6,
                linewidth=1,
                label=train_label if train_label not in added_labels else "",
            )
            plt.plot(
                val_x_values,
                val_y_values,
                ...
                alpha=0.6,
                linewidth=1,
                label=val_label if val_label not in added_labels else "",
            )
    ...
    if display_mode in ["both", term1_label]:
        term1_train_x, term1_train_y, term1_val_x, term1_val_y = plot_file_set(
            term1_train_files, term1_val_files, "blue", "red", "-", term1_label
        )
        plot_mean_val(term1_val_x, term1_val_y, "black", "-", f"{term1_label} Val Mean")

    if display_mode in ["both", term2_label]:
        term2_train_x, term2_train_y, term2_val_x, term2_val_y = plot_file_set(
            term2_train_files, term2_val_files, "purple", "orange", "--", term2_label
        )
        plot_mean_val(
            term2_val_x, term2_val_y, "black", "--", f"{term2_label} Val Mean"
        )

    plt.title(f"{metric} over {x_axis.capitalize()}")
    x_axis_label = (
        f"{x_axis.capitalize()} (minutes)"
        if x_axis == "duration"
        else x_axis.capitalize()
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot training and validation metrics over epochs or duration."
    )
    parser.add_argument(
        "directory", type=str, help="Directory containing the metrics files"
    )
    parser.add_argument(
        "metric", type=str, help="Metric to plot (e.g., f1, precision, recall)"
    )
    parser.add_argument(
        "x_axis",
        type=str,
        choices=["epoch", "duration"],
        help="X-axis to plot (epoch or duration)",
    )
    parser.add_argument(
        "term1_string",
        type=str,
        help='String to identify first term approach (e.g., "baseline")',
    )
    parser.add_argument(
        "term2_string",
        type=str,
        help='String to identify second term approach (e.g., "adamw")',
    )
    parser.add_argument(
        "output_file", type=str, help="Output file to save the plot as PNG"
    )
    parser.add_argument("display_mode", type=str)

    args = parser.parse_args()

    term1_train_files, term1_val_files, term2_train_files, term2_val_files = (
        find_matching_files(args.directory, args.term1_string, args.term2_string)
    )
    plot_metrics(
        term1_train_files,
        term1_val_files,
