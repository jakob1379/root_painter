...
from .test_utils import log_metrics
...
bp_annot_dir = os.path.join(
    sync_dir, "projects", "biopores_corrective_a", "annotations"
)
dense_root_annot_dir = os.path.join(
    sync_dir, "projects", "roots_dense_a", "annotations"
)
corrective_root_annot_dir = os.path.join(
    sync_dir, "projects", "roots_corrective_a", "annotations"
)
dense_nodules_annot_dir = os.path.join(
    sync_dir, "projects", "nodules_dense_a", "annotations"
)
...
def setup_function():
    ...
    # TODO: import copy function across and import.
...
def training(dataset_dir, annot_dir, name):
    ...
    for run in range(1, runs + 1):
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr=0.01, momentum=0.99, nesterov=True
        )
...
def dense_roots_training():
    training(
        annot_dir=dense_root_annot_dir,
        name="root_dense_baseline",
...
def dense_nodules_training():
    training(
        annot_dir=dense_nodules_annot_dir,
        name="nodules_dense_baseline",
        dataset_dir=nodules_dataset_dir,
    )


def corrective_roots_training():
    training(
        annot_dir=corrective_root_annot_dir,
        name="root_corrective_baseline",
        dataset_dir=root_dataset_dir,
    )
